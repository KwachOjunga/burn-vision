...
impl AlexNetConfig {
    pub fn init<B: Backend>(
        ...
        let activation = match activation {
            "relu" => Activations::RELU(Relu),
            _ => Activations::RELU(Relu),
        };
        let features = FeatureExtractorConfig::init::<B>(activation, device);
...
// [TODO] there's an absolute lack of flexibility in the structure of this model;
// (a) __there is absolutely no avenue of altering the internals of the Feature
//     extractor unless one accesses the library file.
// (b) the activation function is fixed, one cannot change the model to have a use case that offers
//     procesing of 3d instead of 2D. ----Currently a SHITSHOW.

pub struct FeatureExtractorConfig;

impl FeatureExtractorConfig {
    pub fn init<B: Backend>(activation: Activations, device: &B::Device) -> FeatureExtractor<B> {
        FeatureExtractor {
...
impl AlexNetClassifierConfig {
    pub fn init<B: Backend>(device: &B::Device, num_classes: usize) -> AlexNetClassifier<B> {
        AlexNetClassifier {
            ...
            linear3: LinearConfig::new(4096, num_classes)
                .with_bias(true)
                .init(device),
